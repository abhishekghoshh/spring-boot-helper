...
public class S3ServiceV2 {
    ...
    private static final Logger logger = LoggerFactory.getLogger(S3ServiceV2.class);
    ...
    public S3ServiceV2(@Autowired S3Client s3Client,
                       @Autowired S3Presigner s3Presigner,
                       @Value("${cloud.aws.s3.bucket.v2}") String bucketName) {
    ...
    public String generatePutPresignedUrl(String filePath, AccessType accessType) {
        ...
        logger.info("Generating presigned PUT URL for file: {} with access type: {} for bucket {}", filePath, accessType, bucketName);

        // Set ACL to public-read if access type is PUBLIC
    ...
    public List<MultiplePreSignedUrlResponse> generateMultiplePreSignedUrls(List<MultiplePreSignedUrlRequest> requests, AccessType accessType) {
        ...
        for (MultiplePreSignedUrlRequest request : requests) {
            ...
            String url = generatePutPresignedUrl(filename, accessType);
